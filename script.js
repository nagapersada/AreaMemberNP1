...
// --- BAGIAN INI YANG DIPERBAIKI (LOGIKA TOMBOL MATI) ---
document.addEventListener('DOMContentLoaded', async () => {
    const isLoggedIn = sessionStorage.getItem('isLoggedIn');
    const loginBtn = document.getElementById('loginButton');

    // Jika belum login dan sedang BUKAN di halaman login, tendang ke index
    ...
    // DETEKSI HALAMAN BERDASARKAN ELEMEN (LEBIH AKURAT)
    if (loginBtn) {
        // Halaman Login
        loginBtn.addEventListener('click', doLogin);
    } 
    else if (document.getElementById('mName')) {
        // Halaman Dashboard
        renderDashboard();
    } 
    else if (document.getElementById('membersTableBody')) {
        // Halaman List Anggota
        prepareMyTeamData();
        initList();
    } 
    else if (document.getElementById('networkDiagram')) {
        // Halaman Network
        prepareMyTeamData();
        initNetwork();
    }
...
// FUNGSI PIALA FINAL (VIP HISTORIS + GROWTH + TOMBOL BERFUNGSI)
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            ...
            // CEK VIP HISTORIS: Apakah saat periode berakhir dia punya 5 direct downline?
            const historicalDirects = globalData.filter(d => 
                d.upline === mem.uid && new Date(d.joinDate) <= endP
            ).length;

            if (historicalDirects < 5) return; // Belum VIP saat itu -> SKIP
