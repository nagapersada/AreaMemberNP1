...
// HITUNG VIP DALAM TIM (Bisa cek masa lalu dengan dateLimit)
function countSpecificVipInTeam(teamMembers, targetLevel, dateLimit=null) {
    let count = 0;
    for (let i = 1; i < teamMembers.length; i++) {
        // Rekursif: Cek pangkat downline pada tanggal tersebut juga
        const downlineRank = getRankLevel(teamMembers[i].uid, dateLimit); 
...
// LOGIKA PANGKAT (Support Time Travel / Cek Masa Lalu)
function getRankLevel(a, dateLimit=null){
    // Ambil tim, jika ada dateLimit, filter member yang join SETELAH tanggal itu
    let teamMembers = [globalData.find(member => member.uid === a), ...getDownlinesRecursive(a)];
    
    if (dateLimit) {
        teamMembers = teamMembers.filter(m => new Date(m.joinDate) <= dateLimit);
    }

    const totalMembers = teamMembers.length;
    
    let directDownlines = globalData.filter(b=>b.upline===a);
    if (dateLimit) {
        directDownlines = directDownlines.filter(m => new Date(m.joinDate) <= dateLimit);
    }
    const directDownlinesCount = directDownlines.length;

    // Pass dateLimit ke fungsi hitung sub-vip
    const vip2InTeam = countSpecificVipInTeam(teamMembers, 2, dateLimit); 
    const vip1InTeam = countSpecificVipInTeam(teamMembers, 1, dateLimit);
...
function countVipStats(a){
    ...
    a.forEach(m=>{
        let c=getRankLevel(m.uid); // Tanpa dateLimit (Rank Sekarang)
        if(c>=1&&c<=9){
            b[c]++;
            vipLists[c].push(m);
...
// FUNGSI PIALA: SUDAH DIREVISI (CEK PANGKAT PADA SAAT PERIODE TERSEBUT)
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            // Jangan cek orang yg join setelah periode berakhir (belum ada saat itu)
            ...
            // Jika Growth > 0 dan mencapai Target, BARU KITA CEK PANGKATNYA SAAT ITU
            if (grow >= target && grow > 0) {
                // PENTING: Cek Rank menggunakan 'endP' sebagai batas waktu
                // Ini akan mensimulasikan rank dia pada akhir periode tersebut
                const rankAtTime = getRankLevel(mem.uid, endP);
