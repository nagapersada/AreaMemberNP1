...
// ====================================================================
// PERBAIKAN LOGIKA GROWTH 50% (SESUAI PERINTAH)
// Syarat Lolos:
// 1. Growth >= Target (Tumbuh 50% dari base)
// 2. Growth > 0 (Harus ada member baru)
// 3. Status VIP di MASA LALU (Pada saat periode berakhir, Referral >= 5)
// ====================================================================
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            // 1. Abaikan member yang baru join SETELAH periode ini berakhir
            if (new Date(mem.joinDate) > endP) return;

            const dls = getDownlinesRecursive(mem.uid);
            
            // 2. Base: Jumlah tim SEBELUM periode dimulai (Minimal 1)
            const base = dls.filter(dl => new Date(dl.joinDate) < startP).length + 1;
            
            // 3. Growth: Jumlah tim yang bergabung SELAMA periode berlangsung
            const grow = dls.filter(dl => {
                const jd = new Date(dl.joinDate);
                return jd >= startP && jd <= endP;
            }).length;
            
            // 4. Target: 50% dari Base (Pembulatan ke bawah)
            const target = Math.floor(base / 2);
            
            // 5. Cek Rank Saat Ini (Hanya untuk label tampilan)
            const currentRank = getRankLevel(mem.uid);
            
            // 6. LOGIKA KUNCI: CEK STATUS VIP DI MASA LALU
            // Hitung jumlah referral langsung (Direct Downline) yang dimiliki TEPAT SAAT periode berakhir.
            // Jika >= 5, berarti saat itu dia sudah memenuhi syarat dasar VIP 1.
            const directDownlinesAtEndP = globalData.filter(b => 
                b.upline === mem.uid && new Date(b.joinDate) <= endP
            ).length;

            // SYARAT FINAL:
            // - Growth >= Target
            // - Growth > 0 (Wajib ada member baru)
            // - directDownlinesAtEndP >= 5 (Saat periode berakhir, dia SUDAH jadi VIP)
            if (grow >= target && grow > 0 && directDownlinesAtEndP >= 5) {
