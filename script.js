...
async function loadData() {
    try {
        // PERBAIKAN DISINI: Menambahkan .range(0, 9999) agar data >1000 tetap terambil
        const [membersRes, historyRes, achieverRes, failureRes] = await Promise.all([
            db.from('members').select('*').range(0, 9999),
            db.from('vip_history').select('*').range(0, 9999),
        ...
        globalData = membersRes.data.map(a => ({
            // Prioritaskan Huruf Besar (Sesuai Screenshot Database Anda)
        ...
        // Jangan alert error koneksi agar tidak mengganggu UX jika hanya delay
    }
}
...
function checkGrowthCondition(uid, teamMembers, minMembersReq) {
    ...
    return (p1Stats.target - p1Stats.actual) <= 0 && (p2Stats.target - p2Stats.actual) <= 0;
}
...
function getPeriodFromDate(date) {
    ...
    if (d >= 16) {
        ...
        start = new Date(py, pm, lastDayPrev); 
...
// ============================================================
// LOGIKA TIM PER LEVEL (VIP MODAL) + FIX ESTIMASI TANGGAL
// ============================================================

// [FIX] Helper Logika Cerdas: Hitung Tanggal Tembus VIP
function getEstAchieveDate(member, rank) {
    const hist = globalHistory.find(h => h.uid === member.uid && h.vip_level === rank);
    if (hist) return new Date(hist.achieved_at);

    if (rank === 1) {
        const directs = getDirectDownlines(member.uid).sort((a,b) => a.joinDate - b.joinDate);
        if (directs.length >= 5) {
            return directs[4].joinDate; // Tanggal join downline ke-5
        }
    }
...
function countVipStats(){
    ...
    myTeamData.forEach(m => {
        ...
        if (r >= 1 && r <= 9) {
            ...
            let achieveTime = getEstAchieveDate(m, r);
...
window.openVipModal = function(level) {
    ...
    const list = vipLists[level].sort((a,b) => {
        const timeA = getEstAchieveDate(a, level);
        const timeB = getEstAchieveDate(b, level);
        return timeB - timeA; 
    });
    ...
    if (list.length > 0) {
        list.forEach(m => {
            ...
            const dObj = getEstAchieveDate(m, level);
            const dateStr = `${dObj.getDate()}/${dObj.getMonth()+1}/${dObj.getFullYear()}`;
            
            const isQualifyDate = (dObj.getTime() !== m.joinDate.getTime());
            const labelDate = isQualifyDate ? "Tembus VIP:" : (level===1 ? "Tembus VIP*:" : "Bergabung:");
...
window.openAchieverModal = function() {
    ...
    // Ambil nama bulan dari END Date periode sebelumnya
    const prevP = getPreviousPeriodDates(currP.start); 
...
// ============================================================
// FITUR SLIDER & DATA KHUSUS (HISTORY & ROADMAP90)
// ============================================================
...
function moveSlide(direction) {
    ...
    let dotIndex = slideIndex; if (dotIndex === 3) dotIndex = 0;
    updateDots(dots, dotIndex);
}
...
function touchMove(event) { if (isDragging) { } }

function touchEnd(event) {
    isDragging = false;
    const movedBy = event.changedTouches[0].clientX - startPos;
    if (movedBy < -50) moveSlide(1);
    else if (movedBy > 50 && slideIndex > 0) moveSlide(-1);
    startAutoSlide();
}


function calculateSlideData(myUid) {
    const histContainer = document.getElementById('historyList');
    let histHTML = '';
    
    const now = new Date();
    let currentP = getPeriodFromDate(now);
    let checkDate = new Date(currentP.start.getTime() - 10000); 
    
    let periodsFound = 0;
    while (periodsFound < 14) {
        const p = getPeriodFromDate(checkDate);
        const userObj = globalData.find(u => u.uid === myUid);
        if(!userObj) break;
        if (p.end < userObj.joinDate) break;

        const stats = calculatePeriodGrowth(myUid, p.start, p.end);
        const isSuccess = stats.actual >= stats.target;
        const pName = getMonthName(p.end.getMonth());
        const pNum = (p.end.getDate() > 15 ? '2' : '1');
        const pYear = p.end.getFullYear();
        
        histHTML += `
        <div class="hist-card ${isSuccess ? 'success' : 'fail'}">
            <div class="hist-info">
                <span class="hist-date">${pName} ${pYear} - PERIODE ${pNum}</span>
                <span class="hist-badge ${isSuccess ? 'bg-ok' : 'bg-no'}">
                    ${isSuccess ? 'TERCAPAI' : 'GAGAL'}
                </span>
            </div>
        ...
        checkDate = new Date(p.start.getTime() - 10000);
        periodsFound++;
    }

    if(histHTML === '') histHTML = '<div style="text-align:center; padding-top:20px; color:#666;">Belum ada riwayat.</div>';
    histContainer.innerHTML = histHTML;


    const rmContainer = document.getElementById('roadmapList');
    const rmCountEl = document.getElementById('rmCount');
    let rmHTML = '';

    let qualifiers = [];

    globalData.forEach(member => {
        const directs = getDirectDownlines(member.uid);
        const vipDirects = directs.filter(d => getRankLevel(d.uid) >= 1);

        if (vipDirects.length >= 5) {
            const achieveTimes = vipDirects.map(d => getEstAchieveDate(d, 1).getTime());
            achieveTimes.sort((a,b) => a - b);
            const qualifyTime = achieveTimes[4];

            qualifiers.push({
                member: member,
                count: vipDirects.length,
                qTime: qualifyTime
            });
        }
    });

    qualifiers.sort((a, b) => b.qTime - a.qTime);

    if (qualifiers.length > 0) {
        qualifiers.forEach((q, index) => {
            rmHTML += `
            <div class="rm-card">
                <div class="rm-left">
                    <div class="rm-rank top3">${index + 1}</div>
                    <span class="rm-name">${q.member.name}</span>
                </div>
                <span class="rm-badge">${q.count} JALUR</span>
            </div>`;
        });
        rmCountEl.innerText = qualifiers.length;
    } else {
        rmHTML = '<div style="text-align:center; padding-top:20px; color:#666;">Belum ada Qualifier.</div>';
        rmCountEl.innerText = 0;
    }
    rmContainer.innerHTML = rmHTML;
}
