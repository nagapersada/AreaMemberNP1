...
// ==========================================
// PERBAIKAN INISIALISASI HALAMAN (LEBIH STABIL)
// ==========================================
document.addEventListener('DOMContentLoaded', async () => {
    const isLoggedIn = sessionStorage.getItem('isLoggedIn');
    const loginBtn = document.getElementById('loginButton');
    const dashboardEl = document.getElementById('mName'); // Ciri khas dashboard
    const listEl = document.getElementById('membersTableBody'); // Ciri khas list
    const networkEl = document.getElementById('networkDiagram'); // Ciri khas network

    // 1. Cek Redirect: Kalau belum login DAN bukan di halaman login, tendang ke index
    if (!isLoggedIn && !loginBtn) {
        window.location.href = 'index.html';
        return;
    }

    // 2. Kalau sudah login, Load Data Global Dulu
    if (isLoggedIn) {
        await loadData();
    }

    // 3. Deteksi Halaman Berdasarkan Elemen (Bukan URL)
    if (loginBtn) {
        // Halaman Login
        loginBtn.addEventListener('click', doLogin);
        // Tambahan: Bisa tekan Enter untuk login
...
async function loadData(){
    try{
        ...
        // Jika error koneksi, beri alert kecil di console atau UI jika perlu
    }
}
...
async function doLogin(){
    const inputUid = document.getElementById('loginUid');
    const a = inputUid.value.trim();
    ...
    b.disabled=true;
    c.innerText = ""; // Reset error

    await loadData(); // Refresh data terbaru dari server

    // Cek apakah data kosong (koneksi error)
    if(globalData.length === 0) {
        c.innerText = "Gagal koneksi server. Coba lagi.";
        b.innerText = "MASUK";
        b.disabled = false;
        return;
    }

    const d = globalData.find(mem => mem.uid === a);
...
function renderDashboard(){
    ...
    // REVISI: Menggunakan Math.floor (Pembulatan ke bawah)
...
// ==========================================
// LOGIKA PIALA GROWTH: PERIODE LALU & VIP SAJA
// ==========================================
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            ...
            // TARGET GROWTH: 50% (Pembulatan ke bawah)
