...
// INISIALISASI (KEMBALI KE ASAL)
document.addEventListener('DOMContentLoaded',async()=>{
    // Cek update Service Worker agar tidak nyangkut file lama
...
async function loadData(){
    try{
        ...
        // 2. Ambil Data History (TAMBAHAN KHUSUS UTK SORTING VIP)
...
// FUNGSI DASHBOARD (KEMBALI KE ASAL)
function renderDashboard(){
    ...
    // Panggil fungsi hitung VIP (yang sudah ada fitur history-nya)
    countVipStats(g); 
    
    const h=new Date,i=h.getDate(),j=h.getMonth(),k=h.getFullYear();let l,m,n,o=!1;
    if(i<=15){
        l=new Date(k,j,1); m=new Date(k,j,15,23,59,59); n=`PERIODE 1 (${getMonthName(j)})`;
    } else {
        l=new Date(k,j,16); m=new Date(k,j+1,0,23,59,59); n=`PERIODE 2 (${getMonthName(j)})`; o=!0;
    }
    
    document.getElementById('currentPeriodLabel').innerText=n;
    const p=g.filter(a=>a.joinDate<l).length, q=g.filter(a=>{let b=new Date(a.joinDate);return b>=l && b<=m}).length, r=Math.ceil(p/2);
...
function renderChart(a,b,c,d){const e=document.getElementById('growthChart').getContext('2d'),f=new Date(b,c,1),g=new Date(b,c,15,23,59,59),h=new Date(b,c,16),i=new Date(b,c+1,0,23,59,59),j=a.filter(a=>a.joinDate>=f&&a.joinDate<=g).length,k=a.filter(a=>a.joinDate>=h&&a.joinDate<=i).length,l=d?'#333':'#D4AF37',m=d?'#D4AF37':'#333';window.myChart&&window.myChart.destroy(),window.myChart=new Chart(e,{type:'bar',data:{labels:['P1','P2'],datasets:[{label:'Growth',data:[j,k],backgroundColor:[l,m],borderColor:'#D4AF37',borderWidth:1}]},options:{responsive:!0,maintainAspectRatio:!1,scales:{y:{beginAtZero:!0,grid:{color:'#333'},ticks:{display:!1}},x:{grid:{display:!1},ticks:{color:'#888',fontSize:9}}},plugins:{legend:{display:!1}}}})}
...
// LOGIKA PANGKAT ASLI (TIDAK DIUBAH)
...
// === FITUR TAMBAHAN: FUNGSI SIMPAN HISTORY ===
async function checkAndSaveHistory(uid, level) {
    ...
    if (!exists) {
        ...
        db.from('vip_history').insert([{ uid: uid, vip_level: level, achieved_at: now }])
          .then(({ error }) => { if (error) console.log("Save history error:", error); });
    }
}
...
function countVipStats(a){
    ...
    a.forEach(m=>{
        ...
        if(c>=1&&c<=9){
            ...
            // 1. Simpan ke history (agar punya tanggal akurat)
            checkAndSaveHistory(m.uid, c);
            
            // 2. Cek Notifikasi (Merah)
...
// === MODAL VIP: DIPERBAIKI (SORTING TERBARU -> TERLAMA) ===
window.openVipModal = function(level) {
    ...
    // LOGIKA SORTING: Menggunakan data history jika ada
    ...
    if (sorted.length > 0) {
        sorted.forEach(m => {
            ...
            body.innerHTML += `<div class="v-item ${is24h ? 'new-name-alert' : ''}">
                <div style="display:flex; flex-direction:column;">
                    <span class="v-n">${m.name} ${is24h ? '🆕' : ''}</span>
...
// === PIALA (GROWTH 50%): LOGIKA DIPERBAIKI AGAR AKURAT ===
window.openAchieverModal = function() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            // 1. Abaikan member yang baru join SETELAH periode berakhir
            if (new Date(mem.joinDate) > endP) return;

            const dls = getDownlinesRecursive(mem.uid);
            
            // 2. Base: Tim SEBELUM periode
            const base = dls.filter(dl => new Date(dl.joinDate) < startP).length + 1;
            
            // 3. Growth: Tim DALAM periode
            const grow = dls.filter(dl => { let jd = new Date(dl.joinDate); return jd >= startP && jd <= endP; }).length;
            
            // 4. Target: 50% dari Base
            const target = Math.floor(base / 2);
            
            // 5. Cek Rank Saat Ini
            const rank = getRankLevel(mem.uid);
            
            // 6. VALIDASI: Cek apakah PADA SAAT ITU dia sudah VIP 1 (Minimal 5 Direct)?
            const directAtTime = globalData.filter(b => b.upline === mem.uid && new Date(b.joinDate) <= endP).length;

            // --- SYARAT FINAL YANG ANDA MINTA ---
            // - Growth >= Target
            // - Growth > 0 (Wajib ada pertumbuhan, biarpun target 0)
            // - Rank >= 1 (Saat ini VIP)
            // - Direct >= 5 (Saat itu juga sudah memenuhi syarat dasar VIP 1)
...
// FUNGSI LAIN (LIST & NETWORK) KEMBALI KE ASLI
