...
// FUNGSI PIALA: PERIODE LALU & VALIDASI VIP HISTORIS
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        title.innerText = `PERAIH 50% - ${plabel}`; achieverTxtContent = `DAFTAR PERAIH GROWTH 50%\n${plabel}\n================\n\n`;
        let achs = []; const myUid = sessionStorage.getItem('userUid');
        
        myTeamData.forEach(mem => {
            // 1. Abaikan member yang bergabung SETELAH periode lalu berakhir
            if (new Date(mem.joinDate) > endP) return;

            // 2. CEK STATUS VIP HISTORIS (SAAT PERIODE BERAKHIR)
            // Syarat VIP 1: Punya minimal 5 Referral Langsung pada saat itu
            const historicalDirects = globalData.filter(d => 
                d.upline === mem.uid && new Date(d.joinDate) <= endP
            ).length;

            if (historicalDirects < 5) return; // SKIP jika belum VIP pada saat itu

            // 3. HITUNG GROWTH (Sama seperti sebelumnya)
            ...
            // TARGET: Pembulatan ke ATAS (Agar konsisten dengan Dashboard)
            const target = Math.ceil(base / 2); 
            const currentRank = getRankLevel(mem.uid);
            
            // 4. SYARAT: Growth >= Target DAN ada Pertumbuhan (>0)
