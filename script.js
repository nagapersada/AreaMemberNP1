...
function countVipStats(a){
    ...
    a.forEach(m=>{
        ...
        if(c>=1&&c<=9){
            b[c]++;
            vipLists[c].push(m);
            // Simpan history jika belum ada
            checkAndSaveHistory(m.uid, c);
            
            // Cek notifikasi (berdasarkan history)
            ...
            // Tandai jika baru (kurang dari 24 jam)
...
// MODAL VIP: Sorting Anggota dari yang TERBARU naik pangkat ke TERLAMA
window.openVipModal = function(level) {
    ...
    // LOGIKA SORTING (Descending by Achieved Date)
...
// ================================================================
// FUNGSI PIALA (ACHIEVER) - REVISI FINAL
// ================================================================
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        // Tentukan PERIODE LALU (Bukan Periode Berjalan)
        if (d > 15) { 
            // Jika tgl 16-31, periode lalu adalah Tgl 1-15 bulan ini
            ...
            // Jika tgl 1-15, periode lalu adalah Tgl 16-30 bulan kemarin
            let pm = m - 1, py = y; 
            if (pm < 0) { pm = 11; py--; } 
        ...
        myTeamData.forEach(mem => {
            // FILTER 1: Hanya cek member yang sudah bergabung SEBELUM periode ini berakhir
            if (new Date(mem.joinDate) > endP) return;

            const dls = getDownlinesRecursive(mem.uid);
            
            // Hitung Base: Jumlah tim SEBELUM periode dimulai (Minimal 1 utk diri sendiri)
            const base = dls.filter(dl => new Date(dl.joinDate) < startP).length + 1;
            
            // Hitung Growth: Jumlah tim yang bergabung DALAM rentang periode
            const grow = dls.filter(dl => {
                const jd = new Date(dl.joinDate);
                return jd >= startP && jd <= endP;
            }).length;
            
            // Hitung Target: 50% dari Base
            const target = Math.floor(base / 2);
            
            // Cek Rank Saat Ini
            const rank = getRankLevel(mem.uid);
            
            // [LOGIKA BARU] Cek apakah pada SAAT ITU (Akhir Periode) dia sudah punya 5 Referral?
            // Ini untuk memastikan dia bukan "VIP Baru" yang baru naik pangkat sekarang.
            // (Asumsi VIP 1 minimal 5 direct downline)
            const directDlsAtThatTime = globalData.filter(b => b.upline === mem.uid && new Date(b.joinDate) <= endP).length;

            // SYARAT LOLOS:
            // 1. grow >= target : Mencapai target
            // 2. grow > 0       : Ada pertumbuhan (Boleh target 0 asal grow 1)
            // 3. rank >= 1      : Sekarang harus status VIP
            // 4. direct >= 5    : Saat periode itu berakhir, minimal sudah punya 5 referral (Sudah VIP saat itu)
            if (grow >= target && grow > 0 && rank >= 1 && directDlsAtThatTime >= 5) {
        ...
        if (achs.length === 0) body.innerHTML = '<div class="v-empty">Belum ada VIP yang mencapai target di periode ini.</div>';
        else {
