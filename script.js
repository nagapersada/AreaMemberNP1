...
// ============================================
// FIX FINAL: CEK PANGKAT PERIODE LALU (RINGAN)
// ============================================
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            ...
            // 2. Data Rank Masa Lalu (Ringan)
            // Syarat VIP 1: Total Tim minimal 5 orang.
            // Kita cek total tim dia PADA SAAT endP.
            // Jika saat itu >= 5, berarti dia SUDAH VIP saat itu.
            const pastTeamSize = dls.filter(dl => new Date(dl.joinDate) <= endP).length + 1;

            // SYARAT:
            // - Growth >= Target
            // - Growth > 0
            // - Past Team Size >= 5 (Artinya sudah VIP 1 di periode lalu)
            if (grow >= target && grow > 0 && pastTeamSize >= 5) {
                // Tampilkan Rank Sekarang (hanya label)
                const rankNow = getRankLevel(mem.uid);
