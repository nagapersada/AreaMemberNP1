...
function renderDashboard(){
    ...
    // LOGIKA PERIODE (DIKEMBALIKAN SEPERTI SEMULA UNTUK STATISTIK)
    const h=new Date,i=h.getDate(),j=h.getMonth(),k=h.getFullYear();let l,m,n,o=!1;
    
    // Variabel m di sini adalah batas CUT-OFF periode LALU (untuk statistik target)
    ...
    // LOGIKA TERPISAH UNTUK COUNTDOWN (TARGET MASA DEPAN)
    let countDownTarget;
    if(i <= 15) {
        // Jika tgl 1-15, hitung mundur ke tgl 15 jam 23:59:59
        countDownTarget = new Date(k, j, 15, 23, 59, 59);
    } else {
        // Jika tgl 16-31, hitung mundur ke akhir bulan
        countDownTarget = new Date(k, j + 1, 0, 23, 59, 59);
    }
    startCountdown(countDownTarget);

    // HITUNGAN STATISTIK TARGET (MENGGUNAKAN LOGIKA ASLI 'm' dan 'l')
    const p=g.filter(a=>a.joinDate<=m).length,q=g.filter(a=>{let b=new Date(a.joinDate);return b.setHours(0,0,0,0),b>=l}).length,r=Math.ceil(p/2);let s=r-q;s<0&&(s=0),document.getElementById('prevPeriodCount').innerText=p,document.getElementById('targetCount').innerText=r,document.getElementById('newMemberCount').innerText=q,document.getElementById('gapCount').innerText=s,renderChart(g,k,j,o)
}
...
// LOGIKA COUNTDOWN REAL-TIME (TIDAK MENGGANGGU FUNGSI LAIN)
let intervalId; 
