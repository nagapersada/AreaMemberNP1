...
function renderDashboard(){
    ...
    // [BARU] Render Target Chart
    renderTargetChart(e);

    const h=new Date,i=h.getDate(),j=h.getMonth(),k=h.getFullYear();let l,m,n,o=!1;31===i?(l=new Date(k,j+1,1),m=new Date(k,j,30,23,59,59),n="PERIODE 1 (BLN DEPAN)"):i<=15?(l=new Date(k,j,1),m=new Date(k,j,0,23,59,59),n=`PERIODE 1 (${getMonthName(j)})`):(l=new Date(k,j,16),m=new Date(k,j,15,23,59,59),n=`PERIODE 2 (${getMonthName(j)})`,o=!0),document.getElementById('currentPeriodLabel').innerText=n;const p=g.filter(a=>a.joinDate<=m).length,q=g.filter(a=>{let b=new Date(a.joinDate);return b.setHours(0,0,0,0),b>=l}).length,r=Math.ceil(p/2);let s=r-q;s<0&&(s=0),document.getElementById('prevPeriodCount').innerText=p,document.getElementById('targetCount').innerText=r,document.getElementById('newMemberCount').innerText=q,renderChart(g,k,j,o)
}

// [BARU] FUNGSI TARGET CHART (DONUT)
function renderTargetChart(currentTotal) {
    // Tentukan target berikutnya: Kelipatan 50 terdekat ke atas
    const nextTarget = Math.ceil((currentTotal + 1) / 50) * 50;
...
// [BARU] FUNGSI GENERATOR BROADCAST
function openBroadcastModal() {
    ...
    const text = `🔥 *UPDATE TIM DV NAGA PERSADA* 🔥\n\n` +
                 ...
                 `"Sukses di crypto bukan soal kecepatan, tapi ketahanan. Tetap solid, tetap fokus!"\n\n` +
                 `🚀 Target Kita: Tumbuh 50-100% periode ini.\n` +
...
function calculateMyRank(currentTeamSize, directDownlineCount, uid) {
    ...
    if (curLevel === 9) { sg = "Top Level"; } 
        ...
        if (cv < next.reqVip) sg = `Kurang ${next.reqVip - cv} @${next.reqLevelName} di Tim`;
    ...
    rg.innerText = sg; rg.style.color = sg.includes("Kurang") ? '#ff4444' : '#ccc';
}
