...
// [MODIFIED] NETWORK WITH VIP STARS & NEW DATE FORMAT
function initNetwork(){
    const a=sessionStorage.getItem('userUid');
    const b=go.GraphObject.make;
    const c=b(go.Diagram,"networkDiagram",{
        padding:new go.Margin(150),
        scrollMode:go.Diagram.InfiniteScroll,
        layout:b(go.TreeLayout,{angle:0,layerSpacing:60,nodeSpacing:20}), 
        undoManager:{isEnabled:!0},
        initialContentAlignment:go.Spot.Center,
        minScale:.1,maxScale:2
    });

    c.nodeTemplate=b(go.Node,"Horizontal",
        {selectionObjectName:"PANEL"},
        // PANEL VERTIKAL: BINTANG DI ATAS, KOTAK DI BAWAH
        b(go.Panel, "Vertical",
            // TEKS BINTANG (VIP)
            b(go.TextBlock,
                {
                    font: "10px sans-serif", stroke: "#ffd700",
                    textAlign: "center", margin: new go.Margin(0,0,2,0),
                    visible: false // Default hidden
                },
                new go.Binding("text", "stars"),
                new go.Binding("visible", "stars", s => s.length > 0)
            ),
            // KOTAK UTAMA (EXISTING)
            b(go.Panel,"Auto",
                {name:"PANEL"},
                b(go.Shape,"RoundedRectangle",{fill:"#000",strokeWidth:1},
                    new go.Binding("stroke","strokeColor"),
                    new go.Binding("strokeWidth","strokeWidth")
                ),
                b(go.TextBlock,{
                    margin:new go.Margin(4,8,4,8),
                    stroke:"#fff",font:"11px sans-serif",
    ...
    const d=myTeamData.map(a=>{
        ...
        const yy=String(b.getFullYear()).slice(-2); // 2 digit tahun
        const dateStr=`${dd}-${mm}-${yy}`;
        
        const vipLvl = getRankLevel(a.uid);
        let starStr = "";
        for(let i=0; i<vipLvl; i++) starStr += "★";

        const isVip = vipLvl >= 1;

        return{
            key:a.uid,
            label:`${a.uid} / ${a.name} / ${dateStr}`,
            strokeColor: isVip ? "#ffd700":"#ffffff",
            strokeWidth: isVip ? 2:1,
            stars: starStr
        }
    });
    ...
    const f=c.findNodeForKey(a);
    f&&(c.centerRect(f.actualBounds),f.isSelected=!0);
