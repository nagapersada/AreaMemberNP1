...
document.addEventListener('DOMContentLoaded',async()=>{
    ...
    if(a.includes('index.html') || document.getElementById('loginButton')){
        const btn = document.getElementById('loginButton');
        if(btn) btn.addEventListener('click',doLogin);
    } 
    else if(a.includes('dashboard.html')){
        renderDashboard();
    } 
    else if(a.includes('list.html')){
        prepareMyTeamData();
        initList();
    } 
    else if(a.includes('network.html')){
        prepareMyTeamData();
        initNetwork();
    }
});
...
function renderDashboard(){
    ...
    myTeamData = g; 
    countVipStats(g); 
    const h=new Date,i=h.getDate(),j=h.getMonth(),k=h.getFullYear();let l,m,n,o=!1;31===i?(l=new Date(k,j+1,1),m=new Date(k,j,30,23,59,59),n="PERIODE 1 (BLN DEPAN)"):i<=15?(l=new Date(k,j,1),m=new Date(k,j,0,23,59,59),n=`PERIODE 1 (${getMonthName(j)})`):(l=new Date(k,j,16),m=new Date(k,j,15,23,59,59),n=`PERIODE 2 (${getMonthName(j)})`,o=!0),document.getElementById('currentPeriodLabel').innerText=n;const p=g.filter(a=>a.joinDate<=m).length,q=g.filter(a=>{let b=new Date(a.joinDate);return b.setHours(0,0,0,0),b>=l}).length;
    
    // DASHBOARD TARGET: CEIL (Konsisten dengan Modal)
    const r=Math.ceil(p/2);
    
    let s=r-q;s<0&&(s=0),document.getElementById('prevPeriodCount').innerText=p,document.getElementById('targetCount').innerText=r,document.getElementById('newMemberCount').innerText=q,document.getElementById('gapCount').innerText=s,renderChart(g,k,j,o)
}
...
function countVipStats(a){
    ...
    for(let i=1;i<=9;i++){
        ...
        if(el){
            el.innerText=b[i]; const parent = el.parentElement;
            if(alertStatus[i]) parent.classList.add('new-alert'); else parent.classList.remove('new-alert');
        }
    }
}
...
window.openVipModal = function(level) {
    ...
    let sorted = [...vipLists[level]].sort((a, b) => {
        const histA = globalHistory.find(h => h.uid === a.uid && h.vip_level === level), histB = globalHistory.find(h => h.uid === b.uid && h.vip_level === level);
        const timeA = histA ? new Date(histA.achieved_at).getTime() : new Date(a.joinDate).getTime(), timeB = histB ? new Date(histB.achieved_at).getTime() : new Date(b.joinDate).getTime();
        return timeB - timeA; 
    });
    if (sorted.length > 0) {
        sorted.forEach(m => {
...
// ==========================================
// 6. PIALA: HISTORICAL CHECK & CEIL TARGET
// ==========================================
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            // Abaikan yg join SETELAH periode target berakhir
            if (new Date(mem.joinDate) > endP) return;

            const dls = getDownlinesRecursive(mem.uid);
            
            // 1. DATA HISTORIS: Total Tim PADA SAAT periode berakhir
            // Hanya menghitung downline yang join SEBELUM atau PAS tanggal akhir periode
            const teamAtPeriodEnd = dls.filter(dl => new Date(dl.joinDate) <= endP).length;
            
            // 2. SYARAT VIP: Punya minimal 5 member PADA SAAT ITU
            // Jika saat itu < 5, dia belum VIP (walaupun skrg mungkin sudah VIP), jadi skip
            if (teamAtPeriodEnd < 5) return; 
            
            // 3. LOGIKA GROWTH (Base & New)
            // Base = Tim sebelum periode dimulai (+1 diri sendiri)
            const base = dls.filter(dl => new Date(dl.joinDate) < startP).length + 1;
            
            // Grow = Member baru di periode itu
            const grow = dls.filter(dl => {
                const jd = new Date(dl.joinDate);
                return jd >= startP && jd <= endP;
            }).length;
            
            // 4. TARGET: Math.ceil (Pembulatan ke Atas)
            // Contoh Awal 0 (Base 1) -> 50% = 0.5 -> Target 1
            const target = Math.ceil(base / 2);
            
            // Lolos jika Grow >= Target (dan ada pertumbuhan)
            if (grow >= target && grow > 0) {
                // Untuk label VIP, kita ambil Rank saat ini saja untuk tampilan (kosmetik)
                const currentRank = getRankLevel(mem.uid);
                achs.push({name: (mem.uid===myUid?mem.name+" (ANDA)":mem.name), uid: mem.uid, target, actual: grow, rank: currentRank});
            }
        });

        achs.sort((a,b)=>b.actual-a.actual);
        
        if (achs.length === 0) body.innerHTML = '<div class="v-empty">Belum ada VIP yang mencapai target periode lalu.</div>';
        else {
