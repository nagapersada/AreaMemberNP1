...
// LOGIN FUNCTION (AMAN - TIDAK AKAN MACET)
...
// PERBAIKAN FATAL DISINI: new go.Binding (BUKAN new Binding)
...
// ============================================
// LOGIKA PERAIH 50% - VIP PERIODE LALU (RINGAN & AKURAT)
// ============================================
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            ...
            // 2. CEK APAKAH SUDAH VIP DI PERIODE LALU? (Tanpa Bikin Berat)
            // Syarat VIP 1: Punya minimal 5 Direct Downline.
            // Kita cek jumlah Direct Downline dia PADA TANGGAL 'endP'.
            const directPast = globalData.filter(u => u.upline === mem.uid && new Date(u.joinDate) <= endP).length;

            // SYARAT FINAL:
            // - Growth >= Target
            // - Growth > 0
            // - directPast >= 5 (Artinya dia sudah punya 5 kaki saat itu = Sudah VIP 1 saat itu)
            if (grow >= target && grow > 0 && directPast >= 5) {
                const rankNow = getRankLevel(mem.uid); // Label Rank pakai data terbaru
