...
let currentGap = 0; // Simpan gap global agar bisa diakses generator
...
function renderDashboard(){
    ...
    // LOGIKA PERIODE (TARGET STATISTIK)
    const h=new Date,i=h.getDate(),j=h.getMonth(),k=h.getFullYear();let l,m,n,o=!1;
    31===i?(l=new Date(k,j+1,1),m=new Date(k,j,30,23,59,59),n="PERIODE 1 (BLN DEPAN)"):i<=15?(l=new Date(k,j,1),m=new Date(k,j,0,23,59,59),n=`PERIODE 1 (${getMonthName(j)})`):(l=new Date(k,j,16),m=new Date(k,j,15,23,59,59),n=`PERIODE 2 (${getMonthName(j)})`,o=!0);
    ...
    else { countDownTarget = new Date(k, j + 1, 0, 23, 59, 59); }
    startCountdown(countDownTarget);

    // FITUR: TAMPILKAN MOTIVASI HARIAN (LOGIN)
    showMotivation();
    ...
    currentGap = s; // Simpan ke global variable untuk dipakai Generator Motivasi
...
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        if (d > 15) { startP = new Date(y, m, 1); endP = new Date(y, m, 15, 23, 59, 59); cutoff = new Date(y, m, 1); plabel = `PERIODE 1 (${getMonthName(m)} ${y})`; } 
        else { let pm = m - 1, py = y; if (pm < 0) { pm = 11; py--; } startP = new Date(py, pm, 16); endP = new Date(py, pm + 1, 0, 23, 59, 59); cutoff = new Date(py, pm, 16); plabel = `PERIODE 2 (${getMonthName(pm)} ${py})`; }
        ...
        if (achs.length === 0) body.innerHTML = '<div class="v-empty">Belum ada VIP yang mencapai target.</div>';
            ...
            achs.forEach((a,i) => { html += `<div class="achiever-item"><div class="achiever-top"><span class="v-n">${i+1}. ${a.name} <small style="color:var(--gold)">(VIP ${a.rank})</small></span><span class="v-u">${a.uid}</span></div><div class="achiever-stats"><span>Target: <b class="val-target">${a.target}</b></span><span>Capaian: <b class="val-actual">${a.actual}</b></span></div></div>`; achieverTxtContent += `${i+1}. ${a.name} (${a.uid}) - VIP ${a.rank}\n   Target: ${a.target} | Capai: ${a.actual}\n\n`; });
            body.innerHTML = html;
        }
    }, 100);
}
...
// ----------------------------------------------------
// [FITUR LAMA] MOTIVASI LOGIN (JANGAN DIHAPUS)
// ----------------------------------------------------
const fieryQuotes = [
    "Jangan tidur nyenyak jika targetmu belum tercapai! Mimpimu butuh eksekusi!",
    "Sukses adalah balas dendam terbaik. BUKTIKAN SEKARANG!",
    "Hari ini Anda santai? Ingat, saingan Anda sedang berlari kencang!",
    "Kemiskinan itu menyakitkan. Berjuanglah sekarang atau menderita nanti!",
    "Ubah rasa lelahmu menjadi rupiah! Jangan cengeng!",
    "Jadilah SINGA! Singa tidak menoleh saat anjing menggonggong. MAJU!",
    "Rezeki tidak akan datang ke orang yang malas! REBUT MILIKMU!",
    "Apakah Anda puas hidup begini saja? BERUBAHLA detik ini!",
    "Penonton hanya bisa tepuk tangan. JANGAN JADI PENONTON!"
];
function showMotivation() {
    ...
    if (modal && textEl) {
        const randomQuote = fieryQuotes[Math.floor(Math.random() * fieryQuotes.length)];
...
// ----------------------------------------------------
// [FITUR BARU] GENERATOR PESAN GRUP (SHARE)
// ----------------------------------------------------
const groupQuotes = [
    "🔥 *PERHATIAN SEMUA!* Waktu terus berjalan, target 50% di depan mata! Kita masih kurang *[GAP] member* lagi. AYO GASPOLL, JANGAN MALU-MALUIN TIM! 🚀",
    "⚠️ *DARURAT!* Kita cuma butuh *[GAP] orang* lagi buat tembus target! Masa segini aja nyerah? Ingat bonus, ingat keluarga! MAJU TERUS! ⚔️",
    "📢 *TIM SAYA YANG TANGGUH!* Lihat angka ini: Kurang *[GAP] member* lagi! Jangan tidur sebelum target tercapai. BUKTIKAN KITA BUKAN PECUNDANG! 🦁",
    "⏳ *WAKTU HABIS!* Jangan biarkan tim lain menyalip kita! Cuma sisa *[GAP] member* lagi untuk target 50%. SAYA YAKIN KITA BISA! 🔥",
    "💰 *DUIT SUDAH DI DEPAN MATA!* Target 50% tinggal selangkah lagi, kurang *[GAP] member*. Ayo rapatkan barisan, kejar sampai dapat! 🚀"
];
...
window.generateShareText = function() {
    ...
    const randomQ = groupQuotes[Math.floor(Math.random() * groupQuotes.length)];
    // Ganti placeholder [GAP] dengan angka real
    const finalTxt = randomQ.replace('[GAP]', currentGap);
