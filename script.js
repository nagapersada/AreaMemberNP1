...
function renderDashboard(){
    ...
    // [BARU] Hitung GAP Pertumbuhan 50%
    let gap = r - q;
    if (gap < 0) gap = 0;
    ...
    document.getElementById('gapCount').innerText=gap; // Isi kolom KURANG
    
    renderChart(g,k,j,o)
}

// [BARU] FUNGSI TARGET CHART (Logic: Next Rank Milestone)
function renderTargetChart(currentTotal) {
    // Daftar Milestone Anggota untuk Peringkat VIP 1 - 9
    const milestones = [5, 31, 101, 201, 351, 501, 901, 1601, 3501];
    
    // Cari angka milestone terdekat yang LEBIH BESAR dari total saat ini
    let nextTarget = milestones.find(m => m > currentTotal);
    
    // Jika sudah melebihi VIP 9 (3501), buat target kelipatan 1000
    if (!nextTarget) {
        nextTarget = Math.ceil((currentTotal + 1) / 1000) * 1000;
    }
    ...
    if(window.donutChart) window.donutChart.destroy(); // Hancurkan chart lama jika ada
...
function openBroadcastModal() {
    ...
    const gap = document.getElementById('targetGap').innerText; // Ambil data gap rank
    ...
    const text = `🔥 *UPDATE TIM DV NAGA PERSADA* 🔥\n\n` +
                 ...
                 `🚀 *MISI KITA:* Kurang ${gap} anggota lagi untuk tembus peringkat baru!\n` +
