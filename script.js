...
document.addEventListener('DOMContentLoaded',async()=>{
    ...
    // Logika Redirect Sederhana (Seperti Semula)
...
function renderDashboard(){
    ...
    // UPDATE: Pakai Math.floor agar konsisten
    const r=Math.floor(p/2);
...
// ==========================================
// FUNGSI PIALA: PERIODE LALU & VIP SAJA
// Update: Pakai Math.floor & Periode Mundur
// ==========================================
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        // LOGIKA PERIODE MUNDUR (PREVIOUS PERIOD)
        if (d > 15) { 
            // Jika skrg tgl 16-31, tampilkan Periode 1 (tgl 1-15 bulan ini)
            startP = new Date(y, m, 1); 
            endP = new Date(y, m, 15, 23, 59, 59); 
            plabel = `PERIODE 1 (${getMonthName(m)} ${y})`; 
        } 
        else { 
            // Jika skrg tgl 1-15, tampilkan Periode 2 (tgl 16-akhir bulan lalu)
            let pm = m - 1, py = y; 
            if (pm < 0) { pm = 11; py--; } 
        ...
        myTeamData.forEach(mem => {
            // Filter: Abaikan yang join setelah periode berakhir
            if (new Date(mem.joinDate) > endP) return;

            // Filter: HANYA VIP (Rank Level >= 1)
            ...
            // RUMUS TARGET: 50% Pembulatan ke Bawah (Math.floor)
            const target = Math.floor(base / 2);
            
            // KONDISI LOLOS: Growth mencapai target DAN ada pertumbuhan (>0)
