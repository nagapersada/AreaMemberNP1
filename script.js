...
// ==========================================
// FUNGSI PIALA: PERIODE LALU & SI A LOLOS
// ==========================================
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            if (new Date(mem.joinDate) > endP) return; // Abaikan yg join setelah periode

            const dls = getDownlinesRecursive(mem.uid);
            
            // 1. CEK VIP HISTORIS: Apakah di AKHIR periode tsb dia sudah punya 5 tim?
            // Si A: Awal 0 -> Akhir 5. teamCountAtPeriodEnd = 5 -> Lolos
            const teamCountAtPeriodEnd = dls.filter(dl => new Date(dl.joinDate) <= endP).length;
            if (teamCountAtPeriodEnd < 5) return; 

            // 2. GROWTH
            const baseCount = dls.filter(dl => new Date(dl.joinDate) < startP).length;
            ...
            // 3. TARGET (Math.ceil)
            // Si A: BaseCount 0 -> TotalBase 1 -> Target = ceil(0.5) = 1.
            const totalBaseForTarget = baseCount + 1;
            const target = Math.ceil(totalBaseForTarget / 2);
        ...
        if (achs.length === 0) body.innerHTML = '<div class="v-empty">Belum ada anggota yang mencapai target di periode lalu.</div>';
        else {
