...
// =========================================================================
// LOGIKA AMAN & AKURAT (TANPA BIKIN MACET)
// =========================================================================
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            if (new Date(mem.joinDate) > endP) return; // Skip yang join setelah periode

            const dls = getDownlinesRecursive(mem.uid);
            
            // 1. Hitung Growth sesuai tanggal
            ...
            // 2. CEK STATUS VIP DI MASA LALU (Metode Ringan)
            // Cukup cek: Apakah pada 'endP', dia sudah punya minimal 5 Direct Downline?
            // Ini pengganti rumus 'getRankLevel' yang berat.
            const directPast = globalData.filter(u => u.upline === mem.uid && new Date(u.joinDate) <= endP).length;

            // SYARAT FINAL:
            // - Growth >= Target
            // - Growth > 0
            // - Punya minimal 5 Direct Downline pada SAAT ITU (Indikasi kuat sudah VIP 1)
            if (grow >= target && grow > 0 && directPast >= 5) {
                const rankNow = getRankLevel(mem.uid); // Ambil label rank sekarang untuk tampilan
                achs.push({name: (mem.uid===myUid?mem.name+" (ANDA)":mem.name), uid: mem.uid, target, actual: grow, rank: rankNow});
            }
        });
