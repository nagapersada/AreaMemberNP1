...
function renderDashboard(){
    ...
    // LOGIKA PERIODE (STATISTIK TARGET)
    const h=new Date,i=h.getDate(),j=h.getMonth(),k=h.getFullYear();let l,m,n,o=!1;
    // m = Cut-off periode lalu (untuk hitung target)
    ...
    // LOGIKA COUNTDOWN (REAL-TIME KE DEPAN)
    let countDownTarget;
    if(i <= 15) {
        // Jika skrg tgl 1-15, countdown ke tgl 15 jam 23:59:59
        countDownTarget = new Date(k, j, 15, 23, 59, 59);
    } else {
        // Jika skrg tgl 16-31, countdown ke akhir bulan jam 23:59:59
        countDownTarget = new Date(k, j + 1, 0, 23, 59, 59);
    }
    startCountdown(countDownTarget);

    // HITUNG DATA TARGET (Pakai 'm' dari logika periode lalu)
...
// LOGIKA COUNTDOWN REAL-TIME (SEPARATED)
let intervalId; 
