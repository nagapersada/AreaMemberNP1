...
// ============================================================
// LOGIKA PERINGKAT (CORE) + PENGECUALIAN KHUSUS
// ============================================================

function getRankLevel(uid) {
    if (rankCache[uid] !== undefined) return rankCache[uid];

    // --- AREA PENGECUALIAN KHUSUS (PRIVILEGE) ---
    // Gilang Giyantine (Force VIP 3)
    if (uid === '814417') {
        rankCache[uid] = 3;
        return 3;
    }
...
function calculateMyRank(teamSize, uid) {
    ...
    if (nextRule) {
        if (nextLvl === 1) {
            ...
            // TAMPILAN 2 BARIS (Status & Syarat Lengkap)
...
window.openVipModal = function(level) {
    ...
    if (list.length > 0) {
        list.forEach(m => {
            ...
            // Format Tanggal (Pencapaian/Join)
