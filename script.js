...
async function loadData(){
    try{
        ...
        // 2. Ambil Data History Pangkat (Supaya sorting VIP benar)
...
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            // Abaikan yg join setelah periode berakhir
            ...
            // SYARAT: Growth > 0 DAN Growth >= Target 50%
            if (grow >= target && grow > 0) {
                // CEK PANGKAT HISTORIS (PADA SAAT ITU)
                // Menggunakan fungsi khusus 'getHistoricalRank' agar tidak merusak fungsi login
                const rankAtTime = getHistoricalRank(mem.uid, endP);
...
// FUNGSI BANTUAN KHUSUS UNTUK CEK PANGKAT DI MASA LALU (TIDAK DIPAKAI LOGIN)
function getHistoricalRank(uid, dateLimit) {
    // Filter data global agar seolah-olah kita berada di masa lalu
    // Hanya ambil member yang sudah join sebelum dateLimit
    const validMembers = globalData.filter(m => new Date(m.joinDate) <= dateLimit);

    // Fungsi Downline khusus pakai data yang sudah difilter
    function getDownlinesAtTime(targetUid) {
        let list = [];
        const directs = validMembers.filter(m => m.upline === targetUid);
        directs.forEach(d => {
            list.push(d);
            list = list.concat(getDownlinesAtTime(d.uid));
        });
        return list;
    }

    // Fungsi hitung VIP dalam tim di masa lalu
    function countVipAtTime(teamArr, lvl) {
        let count = 0;
        for (let i = 1; i < teamArr.length; i++) {
            // REKURSIF: Cek pangkat downline pada saat itu juga
            // Hati-hati: ini berat, tapi karena dijalankan saat klik tombol (bukan login), harusnya aman
            if (getHistoricalRank(teamArr[i].uid, dateLimit) >= lvl) count++;
        }
        return count;
    }

    const memberSelf = validMembers.find(m => m.uid === uid);
    if (!memberSelf) return 0;

    const team = [memberSelf, ...getDownlinesAtTime(uid)];
    const total = team.length;
    const directCount = validMembers.filter(b => b.upline === uid).length;

    const v2 = countVipAtTime(team, 2);
    const v1 = countVipAtTime(team, 1);

    const rankTiers = [{level:9,min:3501,reqVip:2,reqLevel:2},{level:8,min:1601,reqVip:2,reqLevel:2},{level:7,min:901,reqVip:2,reqLevel:2},{level:6,min:501,reqVip:2,reqLevel:2},{level:5,min:351,reqVip:2,reqLevel:2},{level:4,min:201,reqVip:2,reqLevel:2},{level:3,min:101,reqVip:2,reqLevel:2},{level:2,min:31,reqVip:2,reqLevel:1}];

    for(const tier of rankTiers) {
        if (total >= tier.min) {
            let currentVips = (tier.level >= 3) ? v2 : v1;
            if (currentVips >= tier.reqVip) return tier.level;
        }
    }
    return (directCount >= 5) ? 1 : 0;
}
