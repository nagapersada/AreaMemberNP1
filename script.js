...
// SUDAH DIPERBAIKI: Menggunakan 'new go.Binding' (Bukan 'new Binding')
...
// ============================================
// LOGIKA PERAIH 50% - VERSI FINAL (FIXED)
// ============================================
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            ...
            // 2. CEK STATUS VIP DI MASA LALU (Metode Ringan & Akurat)
            // Syarat mutlak VIP 1: Punya minimal 5 Direct Downline.
            // Kita hitung Direct Downline dia pada tanggal akhir periode lalu.
            const directPast = globalData.filter(u => u.upline === mem.uid && new Date(u.joinDate) <= endP).length;

            // SYARAT FINAL:
            // - Growth >= Target (Tercapai)
            // - Growth > 0 (Ada pergerakan)
            // - directPast >= 5 (Status VIP sudah diraih di periode lalu)
            if (grow >= target && grow > 0 && directPast >= 5) {
                const rankNow = getRankLevel(mem.uid); // Ambil label rank terbaru
