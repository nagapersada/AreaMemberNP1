...
// --- LOGIKA PERINGKAT (DIPERBAIKI BIAR GA NGACO TEXTNYA) ---
function calculateRank(total, direct) {
    ...
    // 1. Cari Posisi Rank Sekarang Berdasarkan Total
    ...
    if (currentRank) {
        ...
        if (currentIndex > 0) {
            // Index 0 itu VIP 9 (Tertinggi). Index 7 itu VIP 2 (Terendah di array).
            // Kalau sekarang VIP 2 (Index 7), targetnya index 6 (VIP 3).
            let nextRank = ranks[currentIndex - 1];
            
            gap = nextRank.min - total;
            nextGoalText = `Menuju ${nextRank.name}`; // TEKS DIBAWAH V.I.P
    ...
    document.getElementById('rankNextGoal').innerText = nextGoalText; // FIX: Teks mengejar sesuai target
