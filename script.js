...
function countVipStats(a){
    ...
    for(let i=1;i<=9;i++){
        ...
        // [FIXED] Menambahkan class new-alert ke parent elemen (div.vip-item)
        if(el){
            el.innerText=b[i];
...
window.openVipModal = function(level) {
    ...
    if (sorted.length > 0) {
        sorted.forEach(m => {
            ...
            // [FIXED] Animasi class new-name-alert dan Emoji API
...
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        if (achs.length === 0) body.innerHTML = '<div class="v-empty">Belum ada VIP yang mencapai target.</div>';
            ...
            achs.forEach((a,i) => {
                // [FIXED] Mengembalikan Struktur HTML Kartu agar sesuai CSS
                html += `
                <div class="achiever-item">
                    <div class="achiever-top">
                        <span class="v-n" style="font-size:11px;">${i+1}. ${a.name}</span>
                        <span class="achiever-rank-badge">VIP ${a.rank}</span>
                    </div>
                    <div style="font-size:9px; color:#666; margin-bottom:5px;">UID: ${a.uid}</div>
