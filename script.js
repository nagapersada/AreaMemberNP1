...
type: "file",
fileName: "script.js",
fullContent: `const supabaseUrl = 'https://hysjbwysizpczgcsqvuv.supabase.co';
...
// LOGIN (VERSI STANDAR - DIJAMIN CEPAT)
...
// FUNGSI RANKING UTAMA (LEBIH RINGAN UNTUK DASHBOARD)
function countSpecificVipInTeam(teamMembers, targetLevel) {
    let count = 0;
    for (let i = 1; i < teamMembers.length; i++) {
        // PENTING: Untuk login, kita pakai getRankLevel biasa (tanpa cek masa lalu yg berat)
...
function getRankLevel(a){
    ...
    // Hitung VIP tanpa rekursif masa lalu
...
function countVipStats(a){
    ...
    for(let i=1;i<=9;i++){
        ...
        if(el){ el.innerText=b[i]; const parent = el.parentElement; if(alertStatus[i]) parent.classList.add('new-alert'); else parent.classList.remove('new-alert'); }
    }
}

// MODAL VIP: Sort dari yg Terbaru Naik Pangkat ke Terlama
...
function calculateMyRank(currentTeamSize, directDownlineCount, uid) {
    ...
    const curLevel = getRankLevel(uid); const curRank = rankTiers.find(r => r.level === curLevel) || { name: "MEMBER", level: 0 };
    ...
    if (curLevel === 9) { sg = "Top Level"; } else if (next) {
    ...
    document.getElementById('rankName').innerText = curRank.name; const rg = document.getElementById('rankNextGoal'); rg.innerText = sg; rg.style.color = sg.includes("Tambahan") ? '#ff4444' : '#ccc'; document.getElementById('nextLevelGap').innerText = Math.max(0, gap);
}
...
// =========================================================================
// LOGIKA AMAN: HANYA JALAN SAAT KLIK TOMBOL (TIDAK MEMBEBANI LOGIN)
// =========================================================================
function openAchieverModal() {
    ...
    // GANTI TIMEOUT AGAR BROWSER TIDAK FREEZE
    setTimeout(() => {
        ...
        // Tentukan Periode
        if (d > 15) { 
            startP = new Date(y, m, 1); endP = new Date(y, m, 15, 23, 59, 59); plabel = \`PERIODE 1 (\${getMonthName(m)} \${y})\`; 
        } else { 
            let pm = m - 1, py = y; if (pm < 0) { pm = 11; py--; } 
            startP = new Date(py, pm, 16); endP = new Date(py, pm + 1, 0, 23, 59, 59); plabel = \`PERIODE 2 (\${getMonthName(pm)} \${py})\`; 
        }
        ...
        // BUAT SNAPSHOT DATA MASA LALU (Supaya Cepat & Tidak Error)
        // Kita hanya ambil data yg ada pada saat periode itu berakhir
        const pastData = globalData.filter(m => new Date(m.joinDate) <= endP);

        // FUNGSI LOKAL KHUSUS (ISOLASI)
        function getHistoricalRankSafe(uid) {
            function getDownlinesPast(u) {
                let list = [];
                pastData.filter(m => m.upline === u).forEach(d => { list.push(d); list = list.concat(getDownlinesPast(d.uid)); });
                return list;
            }
            function countVipPast(team, lvl) {
                let c = 0;
                // Loop manual, jangan rekursif terlalu dalam jika tidak perlu
                for(let i=1; i<team.length; i++) {
                   // Cek pangkat downline pakai fungsi ini lagi (Recursive Safe karena data sudah dipotong)
                   if(getHistoricalRankSafe(team[i].uid) >= lvl) c++;
                }
                return c;
            }

            const member = pastData.find(m => m.uid === uid);
            if(!member) return 0;
            
            const team = [member, ...getDownlinesPast(uid)];
            const directs = pastData.filter(m => m.upline === uid).length;
            
            const v2 = countVipPast(team, 2); 
            const v1 = countVipPast(team, 1);
            
            const tiers = [{l:9,m:3501,v:2},{l:8,m:1601,v:2},{l:7,m:901,v:2},{l:6,m:501,v:2},{l:5,m:351,v:2},{l:4,m:201,v:2},{l:3,m:101,v:2},{l:2,m:31,v:2,rl:1},{l:1,m:5,v:0}];
            for(const t of tiers) {
                if(team.length >= t.m) {
                    let cv = (t.l>=3)?v2:v1;
                    if(cv >= t.v) return t.l;
                }
            }
            return (directs >= 5) ? 1 : 0;
        }

        myTeamData.forEach(mem => {
            if (new Date(mem.joinDate) > endP) return; // Skip member baru
            ...
            // SYARAT: GROWTH > 0 DAN TARGET TERCAPAI
            if (grow >= target && grow > 0) {
                // BARU KITA JALANKAN FUNGSI BERAT DISINI (SAFE)
                const rankAtTime = getHistoricalRankSafe(mem.uid);
                if (rankAtTime >= 1) {
                    achs.push({name: (mem.uid===myUid?mem.name+" (ANDA)":mem.name), uid: mem.uid, target, actual: grow, rank: rankAtTime});
                }
            }
        });
